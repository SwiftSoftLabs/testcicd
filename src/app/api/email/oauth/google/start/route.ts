  ...
  try {
    ...
    if (!clientId) {
      return NextResponse.json(
        { error: "Google mailbox OAuth is not configured" },
        { status: 503 },
      );
    }
    ...
    const redirectUri = mailOAuthCallbackUrl("google");
    const scope = [
      "openid",
      "email",
    ...
    const url = `https://accounts.google.com/o/oauth2/v2/auth?${auth.toString()}`;
