...
import { closePullBodySchema, ownerRepoParamsSchema } from "@/lib/integrations/git/schemas";
import {
  jsonError,
  mapUpstreamError,
  parseProviderParam,
...
export async function POST(
  ...
  try {
    ...
    const body = closePullBodySchema.parse(await request.json());
    ...
    if (!client.reopenPullRequest) {
      return jsonError(400, "Reopen is not supported for this provider");
    }

    await client.reopenPullRequest(owner, repo, n);
