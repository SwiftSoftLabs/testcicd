...
import { readmeQuerySchema } from "@/lib/integrations/git/schemas";
...
export async function GET(
  ...
  try {
    ...
    const q = readmeQuerySchema.parse(
    ...
    const readme = await client.getReadme(owner, repo, q.ref ?? undefined);
    if (!readme) return jsonError(404, "README not found");
