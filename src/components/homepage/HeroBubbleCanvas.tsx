...
/* ──────────────────────────────────────────────────────────────
   Hero-only soda bubble field
   Crisp circular bubbles spawn along the bottom edge and drift
   slowly upward with a gentle horizontal wobble.
   ────────────────────────────────────────────────────────────── */

interface HeroBubbleCanvasProps {
  primaryColor: string;
}
...
export default function HeroBubbleCanvas({
  primaryColor,
}: HeroBubbleCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Match jellyfish vertical range so "bottom" is the hero leading edge
    const Y_RANGE = 9;
    const bottomY = -Y_RANGE;
    const topY = Y_RANGE;
    const halfW = 7.2;
    const INIT_BOTTOM_BAND = 2.8;
    const RESPAWN_BOTTOM_BAND = 0.45;
    const RISE_SPEED_MIN = 0.0045;
    const RISE_SPEED_RANGE = 0.0055;

    const randomRiseSpeed = () => RISE_SPEED_MIN + Math.random() * RISE_SPEED_RANGE;
    ...
    const pMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          // Crisp soda bubble: hard rim ring, tiny specular, mostly clear fill
