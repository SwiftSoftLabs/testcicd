create type public.conduit_role            as enum ('Owner', 'Admin', 'Editor', 'Analyst');

create type public.conduit_channel_status  as enum ('active', 'token_expired', 'quota_limit');

create type public.conduit_job_status      as enum ('pending', 'uploading', 'processing', 'scheduled', 'published', 'failed');
...
create index if not exists idx_conduit_workspaces_owner on public.conduit_workspaces(owner_id);
...
create table if not exists public.conduit_workspace_members (
  ...
  role          public.conduit_role not null default 'Editor',
  created_at    timestamptz not null default now(),
  primary key (workspace_id, user_id)
);

create index if not exists idx_conduit_members_user on public.conduit_workspace_members(user_id);
...
create or replace function public.conduit_has_role(ws uuid, min_role public.conduit_role)
returns boolean
language sql
stable
security definer
...
create table if not exists public.conduit_channels (
  ...
  status                   public.conduit_channel_status not null default 'active',
...
create index if not exists idx_conduit_channels_workspace on public.conduit_channels(workspace_id);
...
create index if not exists idx_conduit_assets_workspace on public.conduit_assets(workspace_id);
...
create table if not exists public.conduit_jobs (
  ...
  status              public.conduit_job_status not null default 'pending',
...
create index if not exists idx_conduit_jobs_workspace on public.conduit_jobs(workspace_id);

create index if not exists idx_conduit_jobs_asset     on public.conduit_jobs(asset_id);

create index if not exists idx_conduit_jobs_channel   on public.conduit_jobs(channel_id);

create index if not exists idx_conduit_jobs_scheduled on public.conduit_jobs(scheduled_time);
...
create index if not exists idx_conduit_activity_workspace on public.conduit_activity(workspace_id, created_at desc);
...
on public.conduit_jobs for update
using (public.conduit_has_role(workspace_id, 'Editor'))
with check (
  public.conduit_has_role(workspace_id, 'Editor')
  and (status <> 'published' or public.conduit_has_role(workspace_id, 'Admin'))
...
on public.conduit_jobs for delete
using (public.conduit_has_role(workspace_id, 'Admin'));

create policy "conduit_activity_select"
on public.conduit_activity for select
using (public.conduit_is_member(workspace_id));

create policy "conduit_activity_insert"
on public.conduit_activity for insert
